                ...
                Colormap     color_map)
{
    ...
    drw->visual   = visual;
    drw->depth    = depth;
    drw->color_map     = color_map;
    drw->drawable = XCreatePixmap(dpy, root, w, h, depth);
    drw->picture  = XRenderCreatePicture(dpy, drw->drawable, XRenderFindStandardFormat(dpy, PictStandardARGB32), 0,
                                         NULL);
...
void drw_clr_create(Drw *drw, Clr *dest, const char *clrname, unsigned int alpha)
{
    ...
    if (!XftColorAllocName(drw->dpy, drw->visual, drw->color_map, clrname, dest))
...
Picture drw_picture_create_resized(Drw         *drw,
                                   char        *src,
                                   unsigned int srcw,
                                   unsigned int srch,
                                   unsigned int dstw,
...
int drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad, const char *text, int invert)
{
    ...
    if (!render) {
        ...
        XFillRectangle(drw->dpy, drw->drawable, drw->gc, x, y, w, h);
        d = XftDrawCreate(drw->dpy, drw->drawable, drw->visual, drw->color_map);
        x += lpad;
        w -= lpad;
    }
